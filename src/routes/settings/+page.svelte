<script lang="ts">
	...
	let visibleSecret = $state<string | null>(null);
	...
	$effect(() => {
		if (form?.success && form?.newSecret) {
			visibleSecret = form.newSecret;
			newKeyLabel = '';
		}
	});
	...
	onMount(() => {
		...
		if (savedLang && languages.map(l => l.code).includes(savedLang)) {
			currentLanguage = savedLang as Parameters<typeof setLocale>[0];
			setLocale(savedLang as Parameters<typeof setLocale>[0]);
		}
	});
							...
							<div class="flex items-start">
								...
								<div class="ml-3 w-full">
									...
									<div class="mt-3 flex items-center gap-2">
										...
										<button
											type="button"
											onclick={() => copyToClipboard(visibleSecret!)}
								...
								<tbody class="divide-y divide-slate-200 bg-white">
									{#each data.apiKeys as key}
										<tr>
											...
											>
												{key.createdAt.toLocaleDateString(currentLanguage)}
											</td>
											<td
