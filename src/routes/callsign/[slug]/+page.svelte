<script lang="ts">
    ...
    // Ensure avatarUrl exists in the object structure
    let operator = $state({ 
        avatarUrl: '', // Default if missing
        ...data.operator 
    });
    ...
    function handleImageUpload(e: Event) {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                if (e.target?.result) {
                    operator.avatarUrl = e.target.result as string;
                }
            };
            reader.readAsDataURL(file);

            // 2. Note: In a real app, you might want to upload to a bucket (S3/R2) here 
            // and get a URL back, rather than saving base64 string to the DB.
            // But this works for the "Save Changes" flow below assuming your API handles the string.
        }
    }
        ...
        <nav class="mb-6 flex items-center justify-between">
            ...
            {#if isOwner}
                ...
                >
                    ...
                    {/if}
                    {isEditing ? (isSaving ? 'Saving...' : m.save_changes()) : m.edit_profile()}
                </button>
            {/if}
        </nav>
        ...
        <div class="mb-8 flex flex-col justify-between gap-6 md:flex-row md:items-center">
            <div class="flex items-center gap-6">
                ...
                <div class="relative group">
                    ...
                    <div class="relative h-24 w-24 overflow-hidden rounded-full border-4 border-white bg-slate-200 shadow-md">
                        ...
                        {#if isEditing}
                            <button 
                                ...
                                class="absolute inset-0 flex items-center justify-center bg-black/50 text-white opacity-0 transition hover:opacity-100 group-hover:opacity-100"
                            >
                        ...
                        <div class="relative">
                            {#if flagUrl}
                                <img
                                    ...
                                    class="h-6 w-auto rounded border border-slate-200 shadow-sm"
                                />
                            {/if}
                        </div>
