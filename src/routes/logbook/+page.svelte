    ...
    <main class="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
        <div class="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            ...
            <div class="relative w-full max-w-md">
                <input
                    ...
                    class="w-full rounded-xl border border-slate-200 bg-white py-3 pr-4 pl-10 text-sm shadow-sm transition outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
                />
        ...
        {#if data.qsos.length === 0}
            <div class="rounded-2xl border-2 border-dashed border-slate-200 p-12 text-center">
                <p class="font-medium text-slate-500">{m.no_records_found()}</p>
                <a
                    href="/contact"
            ...
            <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        <thead
                            class="bg-slate-50 text-xs font-bold tracking-wider text-slate-500 uppercase"
                        >
                            <tr>
                            ...
                            {#each data.qsos as qso}
                                <tr class="transition hover:bg-slate-50/50">
                                    <td class="px-6 py-4 font-mono text-slate-500"
                                        >{formatDate(qso.timestamp)}</td
                                    >
                                    <td class="px-6 py-4">
                                        <span class="font-mono text-lg font-black text-slate-900"
                                            >{qso.contactCallsign}</span
                                        >
                                    </td>
                                    <td class="px-6 py-4">{qso.band}</td>
                                    <td class="px-6 py-4">
                                        <span
                                            class="rounded-md bg-blue-50 px-2 py-1 text-xs font-bold text-blue-700"
                                            >{qso.mode}</span
                                        >
                                    </td>
